...
def MLS_func(x, y, ye):
    ...
    try:
        ...
        params, covariance = curve_fit(func, x, y, p0=initial_guess, maxfev=5000, sigma=ye,
                                       absolute_sigma=True)
        k, y_0 = params
        perr = np.sqrt(np.diag(covariance))  # ср.кв.отклонения для параметров

        print(f'Найденные параметры: k = {k}, x_0 = {y_0}')
        ...
        y_grid = func(x_grid, k, y_0)

        plt.plot(x_grid, y_grid, label=f'$y$ = {round(k, 2)}$x$ $-$ {abs(round(y_0, 2))}', color='orange')
        plt.scatter([], [], color='None', label=f'где $k$ = {round(k, 1)}$\\pm${round(perr[0], 1)}')
        plt.xlabel(f'Мощность накачки $P_{{накач}}$, Вт', size=14)
        plt.ylabel(f'Мощность лазера $P_{{излуч}}$, мВт', size=14)
        plt.title(f'Интерполяция функцией вида $y = kx + y_0$', size=14)
...
x = I * 6  # мощность накачки, т.к. U = 6В
y = P * 27
y_e = P * 0.18
